...
- Use `analyze_range_impact` before deleting, overwriting, or restructuring an important worksheet region; it gives a fast blast-radius summary without mutating the workbook, including downstream formulas, validation rules, and conditional formatting expressions that point at the range from elsewhere, through named ranges, or through structured table references.
