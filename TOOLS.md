...
- `analyze_range_impact(filepath: str, sheet_name: str, range_ref: str) -> str`
  Inspects workbook structures that overlap a worksheet range before mutation. Reports intersections with native Excel tables, chart footprints, merged ranges, named ranges, autofilters, print areas, formula cells inside the range, and downstream formulas elsewhere in the workbook that reference it directly, through named ranges, or through structured table references such as `Table1[Sales]`, plus a lightweight `risk_level`.
...
- `read_data_from_excel(filepath: str, sheet_name: str, start_cell: str = "A1", end_cell: Optional[str] = None, max_rows: Optional[int] = None, preview_only: bool = False, compact: bool = False, values_only: bool = False) -> str`
  Returns cell range data with row, column, address, value, and validation metadata under the shared envelope, or a plain 2D `values` matrix when `values_only=True`. Use `max_rows` to paginate tall rectangular ranges without changing the selected columns, and continue from `next_start_row` or `next_start_cell` when the response is truncated.
...
- Use `analyze_range_impact` before deleting, overwriting, or restructuring an important worksheet region; it gives a fast blast-radius summary without mutating the workbook, including downstream formulas that point at the range from elsewhere, through named ranges, or through structured table references.
