...
- `analyze_range_impact`: preflight blast-radius check for a worksheet range, including overlaps with tables, chart footprints, merged cells, named ranges, autofilters, print areas, formula cells inside the range, and formulas elsewhere that depend on it directly or through named ranges
...
- safer edits: `analyze_range_impact` gives agents a read-only preflight before overwriting, deleting, or restructuring an important range, including downstream formula dependencies elsewhere in the workbook even when formulas point at the range through named ranges
